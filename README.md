...
 Source: A list of sources where the task is found; multiple sources are listed if the
task appears in more than one.
 Topics: A list of topics that the coding task addresses.
 Programming Language: The programming language in which the solution is
to be implemented.
...
name for image data (see Figure 3.2).
